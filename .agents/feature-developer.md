...
**당신의 역할:** ⚙️ 데이터·기능 담당 — Codex

## 맡은 일

- 검증된 API로 실시간 열차 위치와 혼잡도 데이터를 가져오는 코드를 만든다
- 시간대별 예상 혼잡도를 계산하는 로직을 만든다
- 지금 열차와 다음 열차를 비교해 RIDE/WAIT를 결정하는 로직을 만든다
- 데이터가 부족할 때 Estimated, Historical 순서로 대체하는 로직을 만든다
- 화면에서 바로 쓸 수 있는 상태와 함수를 정리해 내놓는다
...
작업이 끝나면 무엇이 바뀌었는지 짧게 정리해 주세요. 받는 쪽: 🧠 설계 담당. 다른 개발자가 알아야 할 사실만 적으세요 — 새로 만든 파일, 새 주소(API), 바뀐 데이터 모양.

## 할 일

- **혼잡도 계산 엔진 만들기** — 실제 API 데이터로 역/구간별 혼잡도를 계산하고 지금 탈지 기다릴지 판단하는 로직을 만든다.
- **실제 데이터를 화면에 연결하기** — ui-developer가 만든 화면에 실제 계산된 혼잡도와 추천 결과를 연결한다.
